...
def check_if_signal(symbol):
    try:
        ...
        if isLCC(prepared_df, i - 1) > 0 and prepared_df['close'][69] * 0.007 >= delta_30:
            ...
            if prepared_df['position_in_channel'][i - 1] < POS_IN_CHANNEL and prepared_df['slope'][i - 1] < -SLOPE and mean_atr < 0.7:
                # found a good enter point for LONG
                signal = 'long'

        if isHCC(prepared_df, i - 1) > 0 and delta_30 >= prepared_df['close'][69] * -0.007:
            # found top - OPEN SHORT
            if prepared_df['position_in_channel'][i - 1] > 1 - POS_IN_CHANNEL and prepared_df['slope'][i - 1] > SLOPE and mean_atr < 0.7:
