...
eth_proffit_array = [[round(current_price * 0.01, 3), 2], [round(current_price * 0.014, 3), 2],
                     [round(current_price * 0.018, 3), 2], [round(current_price * 0.022, 3), 2],
                     [round(current_price * 0.026, 3), 1], [round(current_price * 0.03, 3), 1], [round(current_price * 0.03, 3), 0]]
...
def check_if_signal(symbol):
    try:
        ...
        if isLCC(prepared_df, i - 1) > 0:
            ...
            if prepared_df['position_in_channel'][i - 1] < POS_IN_CHANNEL and prepared_df['slope'][i - 1] < -SLOPE and mean_atr < 0.6:
                # found a good enter point for LONG
                signal = 'long'

        if isHCC(prepared_df, i - 1) > 0:
            # found top - OPEN SHORT
            if prepared_df['position_in_channel'][i - 1] > 1 - POS_IN_CHANNEL and prepared_df['slope'][i - 1] > SLOPE and mean_atr < 0.6:
